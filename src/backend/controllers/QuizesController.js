...
/**
 * This handler handles deleting user quiz.
 * send DELETE Request at /api/quiz/:quizId
 * */

export const deleteQuizHandler = function (schema, request) {
	const userId = requiresAuth.call(this, request);
	try {
		if (!userId) {
			return new Response(
		...
		const quizId = request.params.quizId;
		const quizzes = this.db.quizzes.filter((quiz) => quiz._id !== quizId);
		return new Response(200, {}, { quizzes: quizzes });
	} catch (error) {
		return new Response(
			500,
			{},
